...
|---|---|---|---|---|---|
| **Brand** | Display Bold (condensed grotesk) | **92** | −3.0 px (−~33/1000 em) | 1.00 | `ECHO LATTICE` lockup |
...
| **Action** | **IBM Plex Sans Condensed Medium** | **20** (primary **24**) | 0 | 1.20 | Field Index rows (Start / Continue / …) |
| **ActionDisabled** | Medium (same as Action) | same as Action | 0 | 1.20 | Continue w/ no save; locked modes — ink α ≈ 0.32 |
| **Meta** | Mono **iff** size ≤ 13; else body | **12** | 0 | 1.25 | One card line: date · EL code (low contrast) |
...
|---|---|---|
| Brand / Tagline | Display (Bold / SemiBold) only | Mono, body, Medium-as-brand |
| Action / ActionDisabled | **Medium Condensed only** | Mono, body, ThemeDB default |
...
| State | Ink | Geometry |
|---|---|---|
| **Focused** | `rust_fossil` baseline + solid ink tick | Small **solid** margin tick + **single refined baseline** under label advance (text width + 6 px, capped ~240 px) |
| **Hover** (unfocused) | `slate_teal` | Baseline only — same text-width budget, no tick |
| **Idle** | — | **No rule. No circle bullets.** Quiet Medium type on paper |
...
- Jagged / dashed / segmented letterpress rules on selection
- Hollow white circle bullets (binder holes mistaken for row markers)
...
`LedgerChrome.style_index_button` must call Action / ActionDisabled via `LedgerType` (**Medium** face — never mono).  
`LedgerChrome.style_ink_label` at meta sizes must honor the mono ≤ 13 gate.  
`LedgerChrome.draw_index_underlines` implements §4 only (solid baseline + solid tick).
...
2. Field Index actions are **Medium Condensed** at 20/24 — grep-clean of mono on action styling.  
3. Meta at ≤13 uses mono; if a caller requests meta > 13, face falls back to body. One quiet date/EL line.  
4. Screenshot / draw pass: only focused (and hover) rows show solid baseline ink; idle list is clean type.  
5. Field Index plate is sharp paper (clip, soft shadow, subtle fiber) — no torn scribble edge.  
6. Deck compact page_h < 700 uses factor 0.62 sizes without clipping the index plate.
