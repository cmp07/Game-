...
const COLOR_ECHO := Color(0.90, 0.48, 0.22)
const COLOR_CHECKPOINT := Color(0.30, 0.72, 0.78)
const COLOR_CHECKPOINT_USED := Color(0.22, 0.42, 0.48)
const COLOR_WISP := Color(0.72, 0.86, 0.98, 0.70)
...
const COLOR_TELEGRAPH := Color(0.98, 0.82, 0.28, 0.55)
...
func _apply_rewrite() -> void:
	...
	var result := RewriteEngine.apply(
			lattice, sig, recorder.positions(), player_pos, rng)
	if not result.applied:
		status_line = "No rewrite applied (%s)." % result.reason
		_update_hud()
		return
	# Telegraph cells flash before the lattice swap — the fair-preview beat.
	telegraph_cells.clear()
	var tg: Dictionary = result.rewrite.get("telegraph", {})
	for c in tg.get("cells", []):
		telegraph_cells.append(c)
	last_rewrite_cells.clear()
	...
	if not result.combo.is_empty():
		applied_rewrites.append(result.combo)
		for p in result.combo["patches"]:
			last_rewrite_cells.append(p["pos"])
	...
	var combo_txt := ""
	if not result.combo.is_empty():
		combo_txt = " + combo '%s'" % String(result.combo["name"])
	var near_txt := "  [NEAR-MISS]" if result.near_miss else ""
	status_line = "Segment %d — applied '%s'%s (score %.2f, greed %.2f)%s." % [
	...
	for y in range(lattice.height):
		for x in range(lattice.width):
			...
			match cell:
				...
				Lattice.Cell.SOFT:
					color = COLOR_SOFT
				Lattice.Cell.ECHO_WALL:
					color = COLOR_ECHO
				Lattice.Cell.CHECKPOINT:
					color = COLOR_CHECKPOINT
				Lattice.Cell.CHECKPOINT_USED:
					color = COLOR_CHECKPOINT_USED
				Lattice.Cell.WISP:
					color = COLOR_WISP
