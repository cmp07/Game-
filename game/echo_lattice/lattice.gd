...
## (screen coordinates). All lookups take Vector2i.

enum Cell {
	FLOOR = 0,   ## Walkable empty tile.
	WALL = 1,    ## Impassable static wall.
...
const PASSABLE := [Cell.FLOOR, Cell.START, Cell.GOAL, Cell.SOFT]
...
func is_wall(pos: Vector2i) -> bool:
	...
	return c == Cell.WALL or c == Cell.FOSSIL
...
##   '.' floor    '#' wall    'S' start    'G' goal    '*' fossil    ':' soft
