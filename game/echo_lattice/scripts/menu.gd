extends Control
##
## Main menu — VISUAL v2 Steam-hit title card.
...
func _ready() -> void:
	...
	if has:
		subtitle.text = "Chamber %d of %d — field ledger open" % [GameState.current_chamber + 1, ChamberBook.chamber_count()]
	else:
		subtitle.text = "A vertical slice — %d chambers. Ink on paper." % ChamberBook.chamber_count()
...
func _draw() -> void:
	...
	var card := Rect2(page.end.x - 340, page.position.y + 110, 280, 280)
...
func _draw_button_underlines(_card: Rect2) -> void:
	for btn in [continue_button, start_button, quit_button]:
		if btn == null:
			continue
