...
func draw_yard_field_plate(canvas: CanvasItem, plate: Rect2, opts: Dictionary = {}) -> void:
	## True void ambient — deep black with depth, first light, evolving geometry.
	## Not a shed / timber deck / maze film well.
	if plate.size.x < 40.0 or plate.size.y < 40.0:
		return
	var seed: int = int(opts.get("seed", 71))
	var alpha: float = float(opts.get("alpha", 1.0))
	var rng := RandomNumberGenerator.new()
	rng.seed = seed
	var void_far := Color(0.020, 0.024, 0.035, alpha)
	var void_near := Color(0.045, 0.055, 0.075, alpha)
	var light := Color(0.90, 0.88, 0.82, alpha)
	var geo := Color(0.72, 0.78, 0.88, alpha)
	var ink := Color(0.86, 0.88, 0.92, alpha)
	var rust := Color(0.545, 0.227, 0.122, alpha)

	canvas.draw_rect(Rect2(plate.position + Vector2(4, 6), plate.size), Color(0.01, 0.012, 0.02, 0.55 * alpha), true)
	canvas.draw_rect(plate, void_far, true)

	var well: Rect2 = plate.grow_individual(-8.0, -10.0, -8.0, -12.0)
	if well.size.x < 24.0 or well.size.y < 24.0:
		well = plate.grow(-6.0)
	canvas.draw_rect(well, void_near, true)

	# Depth ellipses inside the well.
	var c: Vector2 = well.get_center()
	_draw_plate_ellipse(canvas, c, Vector2(well.size.x * 0.42, well.size.y * 0.38), Color(0.03, 0.035, 0.05, alpha))
	_draw_plate_ellipse(canvas, c + Vector2(0, 6), Vector2(well.size.x * 0.22, well.size.y * 0.20), Color(0.015, 0.018, 0.028, alpha))

	# First light ember.
	canvas.draw_circle(c + Vector2(0, -well.size.y * 0.08), 28.0, Color(light.r, light.g, light.b, 0.10 * alpha))
	canvas.draw_circle(c + Vector2(0, -well.size.y * 0.08), 10.0, Color(light.r, light.g, light.b, 0.22 * alpha))

	# Evolving geometry ribs (creation fill preview).
	for i in range(5):
		var t: float = float(i) / 4.0
		var y: float = well.position.y + 20.0 + t * (well.size.y - 40.0)
		var inset: float = 18.0 + float(i) * 6.0
		canvas.draw_line(
			Vector2(well.position.x + 12.0, y),
			Vector2(c.x - inset, c.y),
			Color(geo.r, geo.g, geo.b, 0.28 * alpha),
			1.1,
			true
		)
		canvas.draw_line(
			Vector2(well.end.x - 12.0, y),
			Vector2(c.x + inset, c.y),
			Color(geo.r, geo.g, geo.b, 0.28 * alpha),
			1.1,
			true
	...
	# Seated luminous span.
	var a: Vector2 = Vector2(well.position.x + well.size.x * 0.28, well.position.y + well.size.y * 0.52)
	var b: Vector2 = Vector2(well.position.x + well.size.x * 0.72, well.position.y + well.size.y * 0.50)
	canvas.draw_line(a + Vector2(0, 4), b + Vector2(0, 5), Color(0.02, 0.03, 0.05, 0.35 * alpha), 3.0, true)
	canvas.draw_line(a, b, Color(light.r, light.g, light.b, 0.85 * alpha), 2.4, true)
	...
	canvas.draw_colored_polygon(span, Color(geo.r, geo.g, geo.b, 0.55 * alpha))

	# Sparse depth motes (rects).
	for i in range(8):
		var p := Vector2(
			well.position.x + rng.randf_range(10.0, well.size.x - 10.0),
			well.position.y + rng.randf_range(12.0, well.size.y - 12.0)
		)
		canvas.draw_rect(Rect2(p, Vector2(1.5, 1.5)), Color(ink.r, ink.g, ink.b, 0.2 * alpha), true)

	canvas.draw_rect(plate, Color(ink.r, ink.g, ink.b, 0.35 * alpha), false, 1.5)
	canvas.draw_rect(Rect2(plate.position.x + 10.0, plate.position.y + 6.0, plate.size.x - 20.0, 2.0), rust, true)


func _draw_plate_ellipse(canvas: CanvasItem, center: Vector2, radii: Vector2, color: Color) -> void:
	var pts := PackedVector2Array()
	for i in range(20):
