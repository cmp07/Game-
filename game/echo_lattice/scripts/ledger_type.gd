...
func role_face(role: String, size_px: int = -1) -> String:
	## Face stack for a role. Meta → mono only when size ≤ META_MONO_MAX_PX.
	match role:
		ROLE_BRAND, ROLE_TAGLINE, ROLE_ACTION, ROLE_ACTION_DISABLED:
			return "display"
		ROLE_DECK:
			return "body"
		ROLE_META:
...
func title_role_scale(page_h: float = 1080.0) -> Dictionary:
	...
	var action: int = role_size(ROLE_ACTION, page_h, false)
