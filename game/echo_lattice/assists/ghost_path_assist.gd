...
extends RefCounted
## Difficulty assist: reveal the chamber ghost path once.
## When Settings → Show ghost path once is enabled, the player may press ghost_assist
## (default G) to paint the intended route a single time per chamber.
...
## Attempt to show path once. `path` is an Array of Vector2i (or Dictionary cells).
...
## Clears the painted ghost without restoring the charge (still once per chamber).
